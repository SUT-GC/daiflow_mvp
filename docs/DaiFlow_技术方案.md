...
DaiFlow 的四个开发阶段（Plan / Todo / Coding / Review）共享同一套 **"AI 生成 + 人机对话 + 左面板联动"** 模式：
...
│     → 注入 xxx_updated 事件 → 左面板实时刷新                │
│                                                           │
│  4. 历史恢复                                               │
│     .jsonl 有 user_message + AI 事件                       │
│     → 重进页面 rebuildMessages() 还原完整对话链              │
│                                                           │
└───────────────────────────────────────────────────────────┘
```
...
| | Plan 阶段 | Todo 拆解阶段 | Coding 阶段 | Review 阶段 |
|--|--|--|--|--|
| session_id | `task:{id}:plan` | `task:{id}:todo_split` | `task:{id}:todo:{todo_id}` | `task:{id}:review` |
| chat 接口 | `POST /tasks/{id}/plan/chat` | `POST /tasks/{id}/todo/chat` | `POST /todos/{id}/chat` | `POST /tasks/{id}/review/chat` |
| Cody session | 新建，存入 `plan_cody_session_id` | 复用 `plan_cody_session_id` | 每个 todo 独立 session | 独立 session |
| 左面板内容 | plan.md（Markdown 渲染） | todo 列表（JSON 渲染） | todo 列表 + diff | 整体 git diff |
| updated 事件 | `plan_updated`：推送 plan.md 全文 | `todo_updated`：推送 todo.json 全文 | `code_updated`：通知前端刷新 diff | `code_updated`：通知前端刷新 diff |
| updated 内容 | content = plan.md 文本 | content = todo.json 字符串 | content = null（前端自行拉 diff） | content = null（前端自行拉 diff） |
| DB 字段更新 | `task.tech_plan` | 无（todo.json 文件） | `todo.status`, `todo.result` | 无 |

> **为什么 Coding/Review 阶段的 updated 事件不推内容？** 因为改的是真实代码仓库的文件（可能涉及多个文件、多个仓库），无法像 plan.md 那样推全文。前端收到 `code_updated` 后调用 `GET /api/tasks/{id}/diff` 重新拉取 diff。
...
// Todo 拆解阶段
function useTodoStage(taskId: string) {
...
// Coding 阶段（当前选中的 todo）
function useCodingStage(taskId: string, todoId: string) {
...
Todo 生成后，用户可以与 AI 讨论调整拆解结果。**对话复用 `plan_cody_session_id`**，与 Plan 阶段共享上下文。

**与 Plan 阶段的差异：**
- updated 事件类型为 `todo_updated`，推送 todo.json 全文（JSON 字符串）
...
Todo 执行完成后，用户可以与 AI 对话补充修改。**每个 todo 独立 Cody session**，互不干扰。

**与 Plan/Todo 阶段的差异：**
- updated 事件类型为 `code_updated`，**不推送文件内容**（代码改动可能涉及多文件多仓库）
...
- [ ] 验证：AI 生成 todo.json，对话可调整拆解，确认后 todos 写入 db，进入编码阶段
